...
function onJson(json) {
  ...
  for (let i = 0; i < num_results; i++) {
    ...
    // Creiamo il pulsante "Like"
    const likeButton = document.createElement('button');
    likeButton.textContent = 'Like';
    likeButton.onclick = () => likeTrack(album_id, title, selected_image);
    // Aggiungiamo immagine, didascalia e pulsante "Like" al div
    album.appendChild(img);
    album.appendChild(caption);
    album.appendChild(likeButton);
...
function likeTrack(album_id, title, image_url) {
  fetch('like_track.php', {
      method: 'POST',
      headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
      },
      body: `album_id=${album_id}&title=${encodeURIComponent(title)}&image_url=${encodeURIComponent(image_url)}`,
  })
